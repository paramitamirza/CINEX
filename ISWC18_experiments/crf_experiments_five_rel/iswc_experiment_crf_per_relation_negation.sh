...
#Label with CRF
java -Xms2g -Xmx8g -jar ../CINEX/CINEXClassifier.jar -c /GW/D5data-8/counting_quantifier/tools/CRF++-0.58/ -p "${name}"_neg -t ./crf_five_rel_feature_data/"${name}"_train_cardinality.data -e ./crf_five_rel_feature_data/"${name}"_eval_neg_test_cardinality.data -m /var/tmp/paramita/models/ -n 100

mv ./crf_five_rel_feature_data/"${name}"_neg_cardinality.out ./crf_five_rel_out/

#Evaluate
java -jar ../CINEX/CINEXEvaluation.jar --prob "${prob}" --compositional --crfeval --ordinals -i ./entity_list_five_rel/eval/val_novalue_"${name}".csv -p "${name}" -f ./crf_five_rel_out/"${name}"_neg_cardinality.out -r ./performance_crf_five_rel_neg.txt -o ./crf_five_rel_predicted/"${name}"_neg.tsv

java -jar ../CINEX/CINEXEvaluation.jar --prob "${prob}" --compositional --crfeval --ordinals -i ./entity_list_five_rel/eval/val_novalue_"${name}".csv -p "${name}"_relaxed -f ./crf_five_rel_out/"${name}"_neg_cardinality.out -r ./performance_crf_five_rel_neg.txt -x  > /dev/null
